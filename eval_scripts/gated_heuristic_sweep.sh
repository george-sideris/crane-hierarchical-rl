#!/bin/bash
# GATED-HEURISTIC parity rows: same protocol, same heuristic, plus the policy-agnostic
# landed (tapered log asset 07-31, density-850, effort 2000). Those rows evaluate a policy on
# not learning - and no policy may claim it. (On real clouds the gate moved 0/26 targets.)
...
for LEVEL in clean 1x 3x 8x; do
  ...
  # shellcheck disable=SC2086
  run "heuristicG_${LEVEL}" --policy_type heuristic --heuristic_dig 0.25 --support_gate 0.25 $N
done
