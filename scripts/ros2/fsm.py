...
Extracted from crane_rl_env_full.py. Pure control logic — no physics or
simulation dependency. Computes EE targets and gripper commands for each phase.
"""
...
class FSMConfig:
    ...
    # Home position in crane base frame. Matches RL env's _home_rear
    # (crane_rl_env_full.py:1255) but with z bumped from 2.0 → 3.0 to give
    # ~1 m more clearance over the trailer surface. Reduces LOWER_TO_DROP
    # descent magnitude and keeps the EE further from trailer poles
    # throughout the deposition phases.
...
class CraneFSM:
    ...
    def tick(self, ee_pos: list, ee_yaw: float, gripper_pos: float) -> EECommand:
        ...
        if self.phase == Phase.HOVER_UP:
            ...
            target_yaw = self.target_yaw
            # ROS1 pattern: command grip_closed in one shot, let PD drive it there.
            ...
            # Go directly to home position. Previously used _hover_pos[2]
            # (grasp_z + hover_clear), which made the crane carry at
            # variable hover-altitude and then LOWER_TO_DROP often timed
            # out before fully descending — the OPEN ended up firing at
            # the wrong (too-high) altitude. Matches RL env behavior.
            drop = self.cfg.drop_position
            target_pos = [drop[0], drop[1], drop[2]]
            target_yaw = self.cfg.drop_yaw  # match ROS1: send_goal(home_xyz, 0.0, ...) during carry
            grip = self.gripper_target
            ...
            if yaw_ok and pos_ok:
                ...
                self.dwell = 0
            if self.dwell >= self.cfg.dwell_count or self.phase_timer >= self.cfg.phase_timeout:
                # Skip LOWER_TO_DROP — auto z adjust during deposit is broken,
                # so just open the gripper at home height after alignment.
            ...
            # Stay at the drop altitude (no descent) — paired with the LOWER_TO_DROP
            # skip in ALIGN_HOME_YAW. The next cycle's HOVER_UP will reposition
            # to the new log's hover above.
            drop = self.cfg.drop_position
            target_pos = [drop[0], drop[1], drop[2]]
            target_yaw = self.cfg.drop_yaw
            grip = self.cfg.grip_open
        ...
        # cmd_pos is in basegrapple frame — jv_controller's ik_xyz already adds
        # the upperpassive offset internally. Adding it here again would
        # double-count and put the crane ~0.415 m too high.
