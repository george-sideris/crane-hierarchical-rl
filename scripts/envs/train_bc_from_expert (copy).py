...
class Expert4D:
    ...
    def get_action(self, env_i: int) -> torch.Tensor:
        ...
        if quat_w is not None:
            ...
            # Get target log's yaw in base frame (state-independent orientation)
            # Policy predicts this, then env converts to joint position at runtime
            yaw_target = env._get_target_log_yaw_in_base_frame(env_i)
        else:
            yaw_target = 0.0
        ...
        # Yaw maps directly to [-pi, pi] -> [-1, 1]
        yaw_norm = max(-0.999, min(0.999, yaw_target / np.pi))
