#!/usr/bin/env python3
"""
Behavioral Cloning with Depth Images

Collects (depth_image, expert_action) pairs and trains a CNN policy.
Uses the same CNN architecture as RL for compatibility with fine-tuning.

Usage:
    ./isaaclab.sh -p crane_testbed/scripts/envs/train_bc_depth.py \
...
class BCCNNPolicy(nn.Module):
    ...
    def __init__(self, obs_dim: int, action_dim: int = 4, dropout: float = 0.0):
        super().__init__()
...
class BCResNetPolicy(nn.Module):
    ...
    def __init__(self, obs_dim: int, action_dim: int = 4, dropout: float = 0.0, latent_dim: int = 256):
        super().__init__()
