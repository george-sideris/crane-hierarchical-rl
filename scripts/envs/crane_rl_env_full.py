...
class CraneDirectEnvFull(DirectRLEnv):
    ...
    def _compute_grapple_stability(self, env_i: int) -> float:
        ...
        raw_stability = grapple_up[2].item()  # Z component of grapple 'up' in world frame

        # Clamp to [0, 1] (negative would mean upside down)
    ...
    def _check_grasped_logs(self, env_i: int, proximity_radius: float = 1.5) -> tuple[int, float, float]:
        ...
        for log_idx in range(start, end):
            ...
            if distance < proximity_radius:
                ...
                raw_alignment = abs(dot_product)  # logs can point either direction
                alignment = raw_alignment ** 8  # very sharp dropoff on misalignment
