...
class BCPointNetPolicy(nn.Module):
    """PointNet policy for BC, outputs 4D action (x, y, z, yaw)."""

    def __init__(self, num_points: int = 1024, action_dim: int = 4,
...
def train_policy(pointclouds: np.ndarray, actions: np.ndarray,
    ...
    # Create model
    policy = BCPointNetPolicy(num_points=num_points, action_dim=4, dropout=dropout).to(device)
