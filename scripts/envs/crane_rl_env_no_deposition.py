...
class CraneDirectEnvCfg(DirectRLEnvCfg):
    ...
    # Strategic state (logs_remaining, cycle_count) can be optionally included - see task configs.
    # NOTE: EE pose is NOT included - it's constant at HOVER_UP (the only decision point)
    observation_space = (max_logs_obs * 4)  # 32*4 = 128 (default Markovian, no strategic state)
...
# ===== Env =====
class CraneDirectEnvNoDeposition(DirectRLEnv):
    """
    ...
    def __init__(self, cfg: CraneDirectEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Debug render: controlled by --viz_markers flag
        self._viz_enabled = args_cli.viz_markers if hasattr(args_cli, 'viz_markers') else False
        self._viz = None
        self._viz_proto_idx = {}
    ...
    def step(self, action: torch.Tensor):
        ...
        for step_count in range(MAX_STEPS):
            ...
            should_render = (self.sim.has_gui() or self.sim.has_rtx_sensors()) and (step_count % 4 == 0)
    ...
    def _calc_optimal_yaw_feedback(self, env_i: int) -> float:
        ...
        if q_log_w is None:
            try:
                ...
                if pos_w is not None and quat_w is not None and hasattr(self, '_per_env_log_counts'):
                    per_env = int(self._per_env_log_counts[env_i])
                    s = env_i * 200  # Each env has 200 slots
                    e = s + per_env  # Only active logs
                    if e > s:
    ...
    def _build_target_selection_obs(self):
        ...
        for env_i in range(N):
            ...
            per_env = int(self._per_env_log_counts[env_i]) if hasattr(self, '_per_env_log_counts') else K
            start = env_i * 200  # Each env has 200 slots
            end = start + min(per_env, K)  # Only active logs, capped at K
            ...
            # Normalize log positions to [-1, 1] using action bounds
            # This makes observation scale match action scale for easier learning
            if self._action_bounds_valid[env_i]:
                min_b = self._action_bounds_min[env_i]  # [3]
                max_b = self._action_bounds_max[env_i]  # [3]
                # Normalize: pos_norm = 2 * (pos - min) / (max - min) - 1
                range_b = (max_b - min_b).clamp(min=1e-6)
                log_pos_norm = 2.0 * (log_pos_b - min_b) / range_b - 1.0
                log_pos_norm = log_pos_norm.clamp(-2.0, 2.0)  # Clamp outliers
            else:
                log_pos_norm = log_pos_b  # Fallback to raw (shouldn't happen)

            # yaw from quaternion (already in [-pi, pi], normalize to [-1, 1])
            yaw = torch.atan2(
                2.0*(log_quat_b[:,0]*log_quat_b[:,3] + log_quat_b[:,1]*log_quat_b[:,2]),
                1.0 - 2.0*(log_quat_b[:,2]*log_quat_b[:,2] + log_quat_b[:,3]*log_quat_b[:,3])
            )  # [num_logs_env]
            yaw_norm = yaw / 3.14159  # Normalize to [-1, 1]

            log_feats = torch.cat([log_pos_norm, yaw_norm.unsqueeze(-1)], dim=-1)  # [num_logs_env, 4]
        ...
        # Add strategic state information if observation space includes it
        # Check if observation space is 132 (with strategic state) or 128 (without)
        include_strategic_state = (self.cfg.observation_space == (K*4 + 4))

        if include_strategic_state:
            # Add strategic state: logs_remaining, cycle_count, prev_logs_grasped, prev_alignment
    ...
    def _heuristic_step(self):
        ...
        for i in range(self.num_envs):
            ...
            if cycle_done is not None and cycle_done[i]:
                ...
                # NO-DEPOSITION MODE: Always use policy (this file is specifically for hierarchical RL)
                ...
                # Debug: show policy action and target at decision points (always, even in hierarchical loop)
                if i == 0:  # Only env0 to avoid spam
                    # Show top 3 log positions from observation (what policy sees)
                    # obs_buf is a dict with "policy" key containing tensor [N, obs_dim]
                    obs = None
                    if hasattr(self, 'obs_buf') and self.obs_buf is not None:
                        if isinstance(self.obs_buf, dict) and "policy" in self.obs_buf:
                            obs = self.obs_buf["policy"][i]
                        elif isinstance(self.obs_buf, torch.Tensor):
                            obs = self.obs_buf[i]
                    log_info = ""
                    total_logs = int(self._per_env_log_counts[i]) if hasattr(self, '_per_env_log_counts') else 200
                    deposited = len(self._deposited_logs[i]) if hasattr(self, '_deposited_logs') else 0
                    log_info = f" [remaining={total_logs - deposited}]"

                    if obs is not None and obs.numel() >= 12:
                        # First 3 logs: each has (x, y, z, yaw), show positions
                        log_positions = []
                        for log_idx in range(min(3, len(obs) // 4)):
                            lx = obs[log_idx * 4].item()
                            ly = obs[log_idx * 4 + 1].item()
                            lz = obs[log_idx * 4 + 2].item()
                            if abs(lz) > 0.01:  # Only show non-zero (existing) logs
                                log_positions.append(f"({lx:.1f},{ly:.1f},{lz:.1f})")
                        if log_positions:
                            log_info += f" TOP-LOGS: {', '.join(log_positions)}"
                        else:
                            # Check if obs is all zeros
                            nonzero = (obs.abs() > 0.01).sum().item()
                            log_info += f" (obs has {nonzero} nonzero values)"
                    else:
                        log_info += " (no obs)"

                    print(f"[env{i}] RAW-ACTION: [{a[0]:.2f}, {a[1]:.2f}, {a[2]:.2f}, {a[3]:.2f}] -> "
                          f"TARGET: x={x_b:.2f}, y={y_b:.2f}, z={z_b:.2f}{log_info}")
                ...
                if yaw_joint_id is not None:
                    ...
                    yaw_target = float(yaw_desired.item())
                # Set yaw target for ALIGN_YAW phase
                self._yaw_targets[i] = yaw_target

                # Target selection is silent - only print grasp results
                ...
                if hasattr(self, '_logs_obj') and self._logs_obj is not None:
                    ...
                    if end > start:
                        ...
                        # Calculate and freeze hover targets
            ...
            if phase == self.PH_HOVER_UP:
                ...
                if self._lift_hold_timer[i] > 0:
                    ...
                    if self._lift_hold_timer[i] == 0:
                        ...
                        reward = self._compute_grasp_reward(logs_grasped, alignment, i)
                        ...
                        print(f"[env{i}] CYCLE {cycle_num}/30 | GRASP: {logs_grasped} logs, alignment={alignment:.2f}, reward={reward:.2f}")
    ...
    def _check_grasped_logs(self, env_i: int, proximity_radius: float = 1.5) -> tuple[int, float]:
        ...
        # Extract basegrapple Y-axis direction (logs should align with grapple Y when yaw-aligned)
        # After yaw alignment: grapple_yaw = log_yaw, so grapple_Y is parallel to log_Y (log length)
        ...
        for log_idx in range(start, end):
            ...
            if distance < proximity_radius:
                ...
                raw_alignment = abs(dot_product)  # Use abs since logs can point either direction
                alignment = raw_alignment ** 8  # Power of 8 for very sharp dropoff on misalignment
    ...
    def _compute_grasp_reward(self, logs_grasped: int, alignment: float, env_i: int) -> float:
        ...
        Returns:
            Reward value: logs_grasped × alignment + bonus - penalty for logs knocked off
        """
        ...
        if logs_grasped == 0:
            # Failed grasp penalty + out-of-bounds penalty
            return -1.0 + (knocked_off * self._logs_out_of_bounds_penalty)
    ...
    def _despawn_grasped_logs(self, env_i: int, proximity_radius: float = 1.5):
        ...
        for log_idx in range(start, end):
            ...
            if distance < proximity_radius:
                ...
                despawn_pos = torch.tensor([0.0, 0.0, -1000.0], device=self.device, dtype=log_pos_w.dtype)
                self._logs_obj.data.root_pos_w[log_idx] = despawn_pos

                # Keep orientation (quaternion) unchanged
                # (Already in root_quat_w, no need to modify)
        ...
        if despawned_count > 0:
            # Concatenate position [N, 3] and quaternion [N, 4] to get full pose [N, 7]
            root_pose = torch.cat([self._logs_obj.data.root_pos_w, self._logs_obj.data.root_quat_w], dim=-1)
            self._logs_obj.write_root_pose_to_sim(root_pose)
...
def main():
    ...
    env = CraneDirectEnvNoDeposition(cfg)
