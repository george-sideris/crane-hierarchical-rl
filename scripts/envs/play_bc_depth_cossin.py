#!/usr/bin/env python3
"""
Play a trained BC depth policy with cos/sin yaw encoding.

This version expects 5D actions: [x, y, z, cos(2*yaw), sin(2*yaw)]
Trained with train_bc_depth_cossin.py

Uses the EXACT same observation function as training for consistency.

Usage:
    ./isaaclab.sh -p crane_testbed/scripts/envs/play_bc_depth_cossin.py \
...
def main():
    ...
    while episodes_done < args_cli.num_episodes and simulation_app.is_running():
        with torch.inference_mode():
            ...
            # Debug: Print action bounds
            if total_grasps == 0:  # Print once at start
                min_b = env._action_bounds_min[0]
                max_b = env._action_bounds_max[0]
                print(f"[BOUNDS] x=[{min_b[0]:.2f}, {max_b[0]:.2f}], "
                      f"y=[{min_b[1]:.2f}, {max_b[1]:.2f}], "
                      f"z=[{min_b[2]:.2f}, {max_b[2]:.2f}]")

            # Print action (5D with cos/sin encoding)
            print(f"[POLICY] Grasp {total_grasps}: obs=[{obs.min():.3f},{obs.max():.3f}], "
                  f"action=[{actions[0,0]:.3f}, {actions[0,1]:.3f}, {actions[0,2]:.3f}, "
                  f"cos={actions[0,3]:.3f}, sin={actions[0,4]:.3f}]")

            # Debug yaw prediction - decode cos/sin to yaw
            yaw_cos = np.tanh(actions[0,3].item())
            yaw_sin = np.tanh(actions[0,4].item())
            policy_yaw = np.arctan2(yaw_sin, yaw_cos) / 2.0  # Decode: atan2(sin, cos) / 2
            correct_yaw = env._get_target_grapple_yaw_b(0)

            # Calculate difference (both in [-π, π])
