...
# Alignment
ax1.plot(theta, alignment_raw, "--", color="C0", linewidth=0.8, alpha=0.5, label=r"$\cos\theta_{\alpha_i}$")
ax1.plot(theta, alignment, color="C0", linewidth=1.5, label=r"$(\cos\theta_{\alpha_i})^8$")
ax1.set_xlabel(r"Misalignment angle $\theta_{\alpha_i}$ (deg)")
ax1.set_ylabel("Score")
ax1.set_title(r"Per-log alignment $\alpha_i$")
...
# Stability
ax2.plot(theta, stability_raw, "--", color="C1", linewidth=0.8, alpha=0.5, label=r"$\max(0,\cos\theta_\varsigma)$")
ax2.plot(theta, stability, color="C1", linewidth=1.5, label=r"$\max(0,\cos\theta_\varsigma)^4$")
ax2.set_xlabel(r"Tilt angle $\theta_\varsigma$ (deg)")
ax2.set_title(r"Stability $\varsigma$")
