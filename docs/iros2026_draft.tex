...
We study grasp target selection for a forestry log loader in simulation, focusing on where and how to orient a grapple to clear dense piles of 200 rigid-body logs in frictional contact.
We introduce a grasp-and-remove formulation in which each environment step is a complete pick attempt: a policy selects a grasp target $(x,y,z,\psi)$, a fixed finite-state machine (FSM) executes the approach and lift, and grasped logs are removed.
Policies observe raw depth-derived point clouds---requiring no semantic segmentation---processed by a PointNet encoder.
We compare four methods: a privileged-state heuristic, from-scratch reinforcement learning (RL) with PPO, behavior cloning (BC) from heuristic demonstrations, and BC-initialized RL fine-tuning (BC+RL) with an asymmetric actor-critic.
From-scratch RL stalls at roughly 60\% pile clearing regardless of observation type or network architecture.
BC achieves $98.6$\% clearing from raw point clouds alone, matching the $97.9$\% heuristic expert.
BC+RL combines BC's full-trajectory coverage with RL's per-grasp optimization, achieving the highest grasp success rate ($87.0$\%) and grapple stability ($0.950$) while maintaining $98.3$\% clearing.
Our results show that raw point clouds are sufficient for sequential grasp targeting and that demonstration data provides the exploration curriculum that from-scratch RL cannot discover.
\end{abstract}
...
We address the grasp target selection problem: given a raw point cloud derived from a stereo depth camera---without semantic segmentation---where should the crane place and orient the grapple to clear the pile efficiently?
...
\begin{itemize}
    ...
    \item A BC$\to$RL fine-tuning pipeline using PointNet on raw (unsegmented) point clouds with an asymmetric actor-critic~\cite{pinto2018asymmetric}, achieving near-expert clearing from depth alone.
    \item A four-method comparison showing: (a) from-scratch RL fails regardless of observation type or architecture, (b) BC achieves near-expert clearing from raw point clouds, and (c) BC+RL improves per-grasp success rate and stability while maintaining clearing performance.
\end{itemize}
...
\label{sec:obs}
The policy observes a raw (unsegmented) point cloud of the scene.
...
In our experiments, we find that raw point clouds match or exceed the performance of segmented point clouds in which a semantic mask isolates log pixels before back-projection (Section~\ref{sec:results}).
This result has practical significance for sim-to-real transfer, as it removes the dependency on a trained segmentation model~\cite{fortin2022instance}.
...
\caption{PPO training curves. Mean episode reward (sum of per-grasp rewards $r = g \cdot \alpha \cdot s$ over a 200-log clearing episode) vs.\ PPO iteration. Each iteration collects $4 N_{\text{env}}$ grasp cycles; flat segments occur when no episode has terminated since the last update. RL (Pose) converges quickly with privileged state; RL (PCD) learns more slowly from point clouds. The BC$\to$RL (PCD) curve initializes from the trained BC policy and reaches the highest episode reward while maintaining pile-clearing performance.}
...
We use an \emph{asymmetric actor-critic} architecture~\cite{pinto2018asymmetric}: the actor observes the raw point cloud (same as BC), while the critic receives the privileged 128-dimensional pose state vector (top-$K$ log poses).
...
Conservative hyperparameters prevent the RL updates from overwriting the BC-learned policy.
We set the initial policy noise $\sigma_0 = 0.05$ (vs.\ $1.0$ for from-scratch RL), reducing exploration to small perturbations around the BC mean.
The learning rate is reduced to $10^{-4}$ (vs.\ $3\!\times\!10^{-4}$), the clipping ratio tightened to $\epsilon = 0.1$ (vs.\ $0.2$), and the entropy coefficient set to zero.
The PointNet encoder learning rate is further scaled by $0.2\times$ to preserve the pre-trained feature representations.
We collect 8 steps per environment per iteration (vs.\ 4 for from-scratch RL) for smoother updates.
...
\begin{table*}[t]
\centering
\caption{Evaluation results across 100 episodes with randomized 200-log pile configurations (mean $\pm$ std). All learned methods use raw (unsegmented) point cloud observations. BC+RL and RL use identical architectures (PointNet actor, state-based critic); the only difference is BC initialization.}
\label{tab:results}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}lllcccccc@{}}
\toprule
Method & Obs. & Arch. & Throughput & Align.\,$\alpha$ & Stab.\,$s$ & Grasp succ.\,(\%) & Clear rate\,(\%) & Knocked off\,(\%) \\
\midrule
Heuristic & Pose & --- & $9.40 \pm 1.17$ & $0.914 \pm 0.027$ & $0.789 \pm 0.024$ & $88.4 \pm 10.1$ & $97.9 \pm 4.4$ & $1.4 \pm 1.6$ \\
\addlinespace
RL        & Raw PCD & Asym & \multicolumn{6}{c}{\TODO{Fill after RL (Raw PCD, asym) evaluation}} \\
\addlinespace
BC        & Raw PCD & --- & $9.23 \pm 0.95$ & $0.905 \pm 0.033$ & $0.824 \pm 0.025$ & $82.5 \pm 10.3$ & $98.6 \pm 2.2$ & \TODO{TBD} \\
\addlinespace
BC$\to$RL & Raw PCD & Asym & $8.66 \pm 0.92$ & $0.908 \pm 0.028$ & $\mathbf{0.950 \pm 0.016}$ & $\mathbf{87.0 \pm 9.3}$ & $98.3 \pm 1.6$ & $1.2 \pm 1.0$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\caption{Clearing progression: fraction of 200-log pile cleared vs.\ grasp attempt number (mean $\pm$ std over 100 episodes). RL plateaus at $\sim$60\% while BC, BC+RL, and the heuristic all approach full clearing.}
...
BC+RL and from-scratch RL use an identical asymmetric actor-critic architecture (PointNet actor on raw PCD, state-based MLP critic), so their performance difference is attributable entirely to BC initialization.

The heuristic expert clears $97.9 \pm 4.4$\% of the pile with a grasp success rate of $88.4$\%.
Both BC ($98.6 \pm 2.2$\%) and BC+RL ($98.3 \pm 1.6$\%) achieve comparable clearing from raw point clouds---notably, without requiring semantic segmentation.
BC+RL achieves the best grasp success rate ($87.0$\%) and grapple stability ($0.950$), demonstrating that RL fine-tuning improves per-grasp execution quality.
From-scratch RL stalls at roughly 60\% clearing despite using the same network architecture.

Qualitatively, we observe three regimes (Fig.~\ref{fig:clearing_curves}):
(i) early clearing, where dense piles yield high-throughput grasps for all methods;
(ii) mid clearing, where RL diverges from the other methods; and
(iii) endgame clearing, where remaining logs form a thin sparse layer and BC+RL's improved success rate becomes most valuable.

\subsection{BC+RL Performance}
\label{sec:bcrl_perf}
BC+RL fine-tuning improves per-grasp metrics while preserving BC's full-trajectory clearing ability.
The most striking improvement is in grapple stability: $0.950 \pm 0.016$ vs.\ $0.824 \pm 0.025$ for BC and $0.789 \pm 0.024$ for the heuristic.
This indicates that RL optimizes the grapple's levelness during lifting, likely by refining the vertical component of the grasp target.
Grasp success rate also improves from BC's $82.5$\% to $87.0$\%, reducing wasted grasps---particularly in the endgame where each miss consumes budget.
Throughput is slightly lower ($8.66$ vs.\ $9.23$ for BC), suggesting that BC+RL favors more conservative, reliable grasps over maximum-throughput grasps.
The mean episode reward of $166.4$ is the highest across all methods, confirming that BC+RL simultaneously improves all reward components.

\subsection{BC Performance}
\label{sec:bc_perf}
BC achieves $98.6 \pm 2.2$\% clearing from raw point clouds, exceeding the heuristic expert's $97.9 \pm 4.4$\% (though the difference is within standard deviations).
Per-grasp quality metrics are close to the heuristic: throughput $9.23 \pm 0.95$ (vs.\ $9.40$), alignment $0.905 \pm 0.033$ (vs.\ $0.914$).
Grasp success rate ($82.5 \pm 10.3$\%) is lower than the heuristic's $88.4$\%, with the gap concentrated in the endgame where sparse remaining logs are harder to localize from depth alone.
...
Throughput, alignment, and stability are computed only over successful grasps ($g > 0$).
The near-match in these conditional metrics indicates that the BC policy has captured the expert's targeting and alignment strategy: when it connects, it grasps just as effectively as the privileged-state expert.
...
To test whether the observation type or architecture could rescue from-scratch RL, we trained four variants: RL with privileged pose observations (128D), segmented PCD, raw PCD (symmetric critic), and raw PCD (asymmetric critic).
All achieve roughly 58--62\% clearing (Table~\ref{tab:rl_ablation}), confirming that RL's failure is fundamental to the exploration landscape rather than a limitation of the visual encoder or critic architecture.

\begin{table}[t]
\centering
\caption{From-scratch RL: observation and architecture ablation. All use multiplicative reward. Clearing rate is mean $\pm$ std over 100 episodes.}
\label{tab:rl_ablation}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}llc@{}}
\toprule
Observation & Architecture & Clear rate\,(\%) \\
\midrule
Pose (128D) & Symmetric & \TODO{Extract from eval} \\
Seg.\ PCD   & Symmetric & $58.6 \pm 4.6$ \\
Raw PCD     & Symmetric & \TODO{Running} \\
Raw PCD     & Asymmetric & \TODO{New run needed} \\
\bottomrule
\end{tabular}
\end{table}

RL~(Seg.\ PCD) clears only $58.6 \pm 4.6$\% of the pile with a grasp success rate of $56.1 \pm 10.0$\%.
...
The policy converges to a narrow targeting pattern that yields adequate reward in dense configurations but fails to adapt as the pile thins.
...
\caption{Per-grasp throughput as a function of remaining logs. From-scratch RL throughput degrades as the pile thins, while the heuristic, BC, and BC+RL maintain higher throughput across pile states.}
\label{fig:remaining_logs}
\end{figure}

\subsection{BC+RL Exploration Noise Sensitivity}
\label{sec:sigma_sweep}
The initial policy noise $\sigma_0$ controls the trade-off between preserving the BC policy and allowing RL exploration.
Table~\ref{tab:sigma_sweep} shows that $\sigma_0 = 0.05$ preserves BC performance while enabling fine-tuning, whereas higher values degrade clearing.

\begin{table}[t]
\centering
\caption{BC+RL sensitivity to initial action noise $\sigma_0$ (all raw PCD, asymmetric AC). Higher noise risks overwriting the BC-learned policy.}
\label{tab:sigma_sweep}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}cccc@{}}
\toprule
$\sigma_0$ & Grasp succ.\,(\%) & Stab.\,$s$ & Clear rate\,(\%) \\
\midrule
0.05 & $87.0 \pm 9.3$ & $0.950 \pm 0.016$ & $98.3 \pm 1.6$ \\
0.1  & \multicolumn{3}{c}{\TODO{Needs run}} \\
0.3  & \multicolumn{3}{c}{\TODO{Needs run}} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Why RL fails from scratch.}
From-scratch RL achieves competitive per-grasp reward but stalls at roughly 60\% clearing regardless of observation type or critic architecture (Table~\ref{tab:rl_ablation}).
...
This failure is not a perception limitation---RL with privileged pose observations fares no better---but rather a consequence of the exploration landscape: the policy never discovers the full diversity of pile states encountered during a complete clearing trajectory.

\textbf{Why BC+RL succeeds.}
...
The dramatic stability improvement ($0.950$ vs.\ $0.824$) suggests that RL learns to refine the vertical positioning of grasps, producing more level lifts.
The conservative hyperparameters ($\sigma_0 = 0.05$, reduced learning rate, tight clipping) are critical: higher exploration noise risks overwriting the BC policy before the fresh critic can provide meaningful value estimates.

\textbf{Raw PCD sufficiency.}
Raw (unsegmented) point clouds achieve $98.6$\% clearing via BC and $98.3$\% via BC+RL, matching or exceeding the segmented-PCD BC baseline ($96.3$\% in our earlier experiments with segmented input).
...
\textbf{BC+RL trade-offs.}
BC+RL's throughput ($8.66$ logs/grasp) is lower than BC's ($9.23$), suggesting a reliability--throughput trade-off: the fine-tuned policy favors more conservative grasps that succeed more often ($87.0$\% vs.\ $82.5$\%) and produce more stable lifts ($0.950$ vs.\ $0.824$), at the cost of slightly fewer logs per grasp.
Whether this trade-off is desirable depends on the application; in practice, the higher success rate and lower variance of BC+RL may be preferred for autonomous operation.
...
Our results show that from-scratch RL fails to clear dense piles regardless of observation type or architecture, stalling at roughly 60\%.
BC trained on raw (unsegmented) point clouds achieves $98.6$\% clearing, matching the $97.9$\% privileged-state heuristic.
BC+RL fine-tuning achieves the strongest overall performance: $87.0$\% grasp success, $0.950$ stability, and $98.3$\% clearing, demonstrating that demonstration data provides the exploration curriculum that RL cannot discover on its own.
The success of raw point clouds---without semantic segmentation---simplifies the perception pipeline for future sim-to-real transfer.
Future work will explore sim-to-real deployment with domain randomization, richer point cloud architectures (e.g., PointNet++, point transformers), and curriculum learning over pile complexity.
