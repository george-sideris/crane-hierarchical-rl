...
This setting is challenging because (i) the scene is densely cluttered with up to 200 rigid-body logs in frictional contact, (ii) each grasp disturbs the pile through contact cascades, continuously reshaping the configuration for subsequent attempts, and (iii) no two pile configurations are alike, so a targeting strategy must be robust to diverse arrangements.
...
We find that from-scratch RL, while effective at maximizing per-grasp reward, develops a narrow targeting strategy that stalls at roughly 60\% pile clearing, failing to adapt as the pile geometry changes.
In contrast, BC trained on heuristic demonstrations from segmented point clouds achieves clearing performance close to the expert, demonstrating that depth-derived geometry is sufficient for effective grasp targeting in this domain.

Our main contributions are:
\begin{itemize}
    \item A grasp-and-remove crane environment in Isaac Lab that supports GPU-parallelized training across many environments with configurable pile sizes and shapes (up to 200 logs in our experiments), paired with a fixed FSM controller that isolates the targeting decision from low-level execution.
...
Our setting differs in important ways: we perform power grasps from dense piles of identical cylindrical logs that are initially stacked in ordered layers.
The homogeneity and tight packing mean that logs are in extensive frictional contact with their neighbors, and each grasp attempt (which captures 5--20 logs simultaneously) disturbs the pile, causing settling, rolling, and potential knock-offs that reshape the configuration for all subsequent attempts.
...
For grasp prediction, Wallin et~al.~\cite{wallin2024multilog} use RL with virtual visual servoing for multi-log grasping from piles of 2--5 logs, separating image segmentation from Cartesian crane control; their agent achieves 95\% success but the pile dynamics are limited to a handful of objects.
F{\"a}lldin et~al.~\cite{falldin2025synthesizing} train a U-Net CNN on synthetic RGB-D images to predict grasp poses for piles of up to 7 logs, with an open-loop kinematic controller; they do not address sequential clearing or pile-scale dynamics.
...
For crane control, Vu et~al.~\cite{vu2025autonomous} present a MuJoCo-based forestry crane simulator and benchmark RL velocity controllers for grasping single logs of varying diameter.
Kurinov et~al.~\cite{kurinov2025reinforcement} train an RL agent for the full single-log pick-and-place pipeline (locate, grasp, transport, load) using Isaac Gym, achieving 94\% success across parallel environments but with one log at a time.
...
These works address individual subtasks but none tackle sequential clearing of a dense pile at the scale considered here (200 logs).

\section{Task Formulation}
\subsection{Grasp-and-Remove Formulation}
\label{sec:formulation}
Each environment step corresponds to a complete grasp attempt rather than a low-level control tick: a policy selects a grasp target, the FSM executes the approach, close, and lift, and grasped logs are removed from the scene.
...
Because a cylindrical log at yaw $\psi$ is identical to the same log at $\psi + \pi$, the policy represents $\psi$ via the $\pi$-periodic encoding $(\cos 2\psi, \sin 2\psi)$, giving a 5D action $a_t \in \mathcal{A} \subset \mathbb{R}^5$ (Section~\ref{sec:actions}).
...
\begin{enumerate}
    ...
    \item \texttt{CLOSE}: Close grapple jaws via discrete stepping.
...
Because a cylindrical log's length direction is arbitrary, we take the absolute value; raising to the 8th power creates a sharp dropoff that penalizes even moderate misalignment:
\begin{equation}
...
\begin{figure}[t]
\centering
\TODO{Insert zoomed-in grasp diagram showing: (1) grapple holding logs with coordinate frames drawn on the grapple ($\hat{y}_g$, $\hat{z}_g$) and a log ($\hat{y}_\ell$), illustrating the alignment dot product $\langle \hat{y}_g, \hat{y}_\ell \rangle$ and the stability dot product $\hat{z}_g^\top \hat{z}_w$; (2) the 3D action space bounding box overlaid on the rack, showing the workspace limits $[b_i^{\min}, b_i^{\max}]$ that define the policy's reachable target space; (3) include outcome\_scores.pdf subplots showing the shaped score curves ($|\cos\theta|^8$ for alignment, $(\cos\theta)^4$ for stability) alongside the diagram.}
