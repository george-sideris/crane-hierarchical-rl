...
First, each pixel $(u,v)$ with a valid depth reading within the sensor range $[d_{\min},\, d_{\max}]$ is back-projected into the camera-frame point cloud $\mathcal{P}^{\mathrm{c}}$ using the intrinsic matrix $\mathbf{K} \in \mathbb{R}^{3 \times 3}$:
\begin{equation}
    \mathcal{P}^{\mathrm{c}} = \big\{\, \mathbf{K}^{-1}(u,v,1)^\top \!\cdot D_t(u,v) \;\big|\; d_{\min} \le D_t(u,v) \le d_{\max} \big\}.
\end{equation}
These camera-frame points are then transformed to the world frame $\mathcal{P}^{\mathrm{w}}$ using the known camera extrinsics.
Second, each point $\mathbf{p} \in \mathcal{P}^{\mathrm{w}}$ is transformed into the crane base frame using the base-frame rotation $\mathbf{R}^{\mathrm{b}}$ and translation $\mathbf{t}^{\mathrm{b}}$:
\begin{equation}
    \mathcal{P}^{\mathrm{b}} = \big\{\, {\mathbf{R}^{\mathrm{b}}}^\top(\mathbf{p} - \mathbf{t}^{\mathrm{b}}) \;\big|\; \mathbf{p} \in \mathcal{P}^{\mathrm{w}} \big\}.
\end{equation}
Third, Farthest Point Sampling (FPS) downsamples $\mathcal{P}^{\mathrm{b}}$ to a fixed-size set of $N_p = 1024$ points, yielding the observation:
\begin{equation}
    o_t = \mathrm{FPS}(\mathcal{P}^{\mathrm{b}},\; N_p) \;\in \mathbb{R}^{N_p \times 3}.
\end{equation}
...
Each position component is squashed via the hyperbolic tangent $\tanh$ and then linearly mapped to the per-environment workspace bounds to produce the base-frame target position $p^{\mathrm{b}}_i$:
\begin{equation}
    p^{\mathrm{b}}_i = \frac{(\tanh \tilde{a}_i + 1)}{2}(b_i^{\max} - b_i^{\min}) + b_i^{\min}, \quad i \in \{x,y,z\},
\end{equation}
...
These bounds define the reachable target space; logs knocked outside them are automatically removed and counted separately (Section~\ref{sec:outcome}). The heuristic expert (Section~\ref{sec:expert}) operates within the same bounds.
...
For BC training, expert targets use the same encoding.
...
The grapple is controlled using a PD velocity law with saturation and discrete stepping to avoid abrupt contact forces.
...
\textbf{Alignment} ($\alpha$): for each grasped log $i$, we compute per-log alignment $\alpha_i$ from the angle between the grapple $\hat{y}^{\mathrm{g}}$ axis and the log's length axis $\hat{y}^{\ell}_i$ via their dot product (${\hat{y}^{\mathrm{g}}}^\top \hat{y}^{\ell}_i = \cos\theta_{\alpha_i}$, where $\theta_{\alpha_i}$ is the misalignment angle).
...
\begin{equation}
    \alpha_i = \big({\hat{y}^{\mathrm{g}}}^\top \hat{y}^{\ell}_i\big)^{8}, \qquad \alpha = \frac{1}{g}\sum_{i=1}^{g} \alpha_i \;\in [0,1].
\end{equation}

\textbf{Stability} ($\varsigma$): the grapple's levelness after lifting, measured as ${\hat{z}^{\mathrm{g}}}^\top \hat{z}^{\mathrm{w}} = \cos\theta_\varsigma$, where $\theta_\varsigma$ is the tilt angle between the grapple's up vector and the world vertical.
...
\begin{equation}
    \varsigma = \big(\max(0,\;{\hat{z}^{\mathrm{g}}}^\top \hat{z}^{\mathrm{w}})\big)^{4} \;\in [0,1].
\end{equation}
...
Position tolerance (up.\ passive / grapple) & 0.10 / 0.25\,m \\
...
At each pick cycle, the expert selects the topmost active log (highest $z$-coordinate), transforms its center into the crane base frame as the $(x,y,z)$ target, and aligns the grapple yaw $\psi$ with the log's length axis (choosing the orientation requiring minimal joint rotation). Yaw alignment is recomputed in the \texttt{ALIGN} phase for closed-loop tracking.
...
Key hyperparameters: $\sigma_0 = 1.0$, clipping ratio $\epsilon=0.2$, entropy coefficient $0.01$, learning rate $3\times10^{-4}$ with adaptive KL-based scheduling (target KL $0.01$), GAE with $\lambda=0.95$, discount factor $\gamma = 0.99$, four steps per environment per iteration, five learning epochs with four mini-batches per epoch, and gradient clipping at norm $1.0$.
...
and the action-space bounding box; the crane articulation, showing all links and joints.}
