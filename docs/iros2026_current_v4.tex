...
\subsection{Metrics}
\label{sec:metrics}
We report metrics derived from the outcome tuple $(g, \alpha, \varsigma)$ (Section~\ref{sec:outcome}), all as per-episode mean $\pm$ std: \emph{grasp success rate} (fraction with $g>0$), \emph{throughput} (mean logs per successful grasp), \emph{alignment} (grasp-count-weighted mean $\alpha$), \emph{stability} (mean $\varsigma$ over successful grasps), \emph{clearing rate} (fraction of logs intentionally removed), and \emph{knocked-off percentage} (fraction knocked outside bounds).
\section{Results}
\label{sec:results}
\begin{table*}[t]
\centering
