...
\title{BARK: Clearing Dense Log Piles from Vision via Behavior Cloning and Reinforcement Learning}
...
The resulting policy achieves $98.3$\% pile clearing while improving grasp quality by $16$\% and grasp success rate by $5$\% over the BC initialization, using only depth-derived point clouds.
\end{abstract}
...
The best RL variant (Raw PCD) achieves only 53\% clearing with 53.3\% grasp success, compared to 98.6\% and 82.5\% for BC (Table~\ref{tab:rl_ablation}).

\begin{table*}[t]
\centering
\caption{Robustness evaluation under domain randomization (DR), observation noise, and their combination (mean $\pm$ std over 100 episodes). DR varies log friction, mass, and spawn pattern. Observation noise ($\sigma_{\mathrm{pos}}=0.10$\,m, consistent with stereo-camera positioning precision in field trials~\cite{semberg2024realtime}) is applied as i.i.d.\ Gaussian perturbations to each point in the input point cloud for learned policies. For the heuristic, noise perturbs the target log position and orientation ($\sigma_{\mathrm{yaw}} = \arctan(\sigma_{\mathrm{pos}} / (L/2)) \approx 3.8^{\circ}$ for $L=3$\,m logs).}
...
Heuristic & Obs.\ noise      & \multicolumn{6}{c}{\TODO{Eval needed}} \\
Heuristic & DR               & \multicolumn{6}{c}{\TODO{Eval needed}} \\
Heuristic & DR + obs.\ noise & \multicolumn{6}{c}{\TODO{Eval needed}} \\
\addlinespace
BC        & Nominal          & $9.23 \pm 0.95$ & $0.905 \pm 0.033$ & $0.824 \pm 0.025$ & $82.5 \pm 10.3$ & $98.6 \pm 2.2$ & $0.8 \pm 0.7$ \\
BC        & DR               & \multicolumn{6}{c}{\TODO{Eval needed}} \\
BC        & Obs.\ noise      & \multicolumn{6}{c}{\TODO{Eval needed}} \\
BC        & DR + obs.\ noise & \multicolumn{6}{c}{\TODO{Eval needed}} \\
\addlinespace
BC$\to$RL & Nominal          & $8.66 \pm 0.92$ & $0.908 \pm 0.028$ & $0.950 \pm 0.016$ & $87.0 \pm 9.3$ & $98.3 \pm 1.6$ & $1.2 \pm 1.0$ \\
BC$\to$RL & DR               & \multicolumn{6}{c}{\TODO{Eval needed}} \\
BC$\to$RL & Obs.\ noise      & \multicolumn{6}{c}{\TODO{Eval needed}} \\
BC$\to$RL & DR + obs.\ noise & \multicolumn{6}{c}{\TODO{Eval needed}} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tabular}{lccc}
\toprule
Variant & Clear.\,\% & Succ.\,\% & Reward \\
\midrule
Baseline ($g \cdot \alpha \cdot \varsigma$) & 53.0 & -- & 49.4 \\
Throughput only ($g$) & TBD & TBD & TBD \\
Normalized ($\frac{g}{\text{avail}} \cdot \alpha \cdot \varsigma$) & TBD & TBD & TBD \\
Curriculum + bonus@60/75/90\% & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
\end{table}
...
Future work will explore sim-to-real deployment with domain randomization, richer point cloud architectures (e.g., PointNet++, point transformers).
