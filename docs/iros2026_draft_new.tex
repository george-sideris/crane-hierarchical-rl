...
In our experiments, from-scratch reinforcement learning (RL) does not discover strategies that effectively clear the pile while maintaining grasp quality, regardless of observation type.
...
BC provides the full-trajectory exploration curriculum that RL alone cannot discover, while RL optimizes per-grasp execution beyond what the demonstrator achieves.
...
\begin{figure}[b]
\centering
\includegraphics[width=\columnwidth]{mill.png}
...
\begin{itemize}
    ...
    \item A four-method comparison (heuristic, RL, BC, BC$\to$RL) showing: (a) from-scratch RL underperforms regardless of observation type, (b) BC achieves near-expert clearing from raw point clouds, and (c) BC+RL improves grasp stability by 15\% over BC and 19\% over the heuristic while maintaining clearing performance.
\end{itemize}
...
\caption{Grasp geometry and action space. Left: coordinate frames used to compute alignment ($\alpha$) and stability ($\varsigma$). Right: the 3D workspace bounds define the policy's action space over the rack.}
...
Telescope / Upper/Lower chain & 25 / 9 kg \\
\addlinespace[4pt]
...
Position tol.\ (upper passive joint / grapple) & 0.10 / 0.25 m \\
...
The BC actor weights (PointNet encoder and actor MLP) are loaded into the PPO actor; the critic is initialized randomly and the optimizer starts fresh (Fig.~\ref{fig:bcrl_pipeline}).
...
\caption{Observation pipeline for a single grasp. (a)~Camera RGB, (b)~raw depth with colorbar, (c)~3D back-projection in the crane base frame (gray: background, color: log points), (d)~FPS-downsampled point cloud with predicted grasp target and grapple footprint (green~=~hit).}
...
\begin{table*}[t]
\centering
\caption{Evaluation results across 100 episodes with randomized 200-log pile configurations (mean $\pm$ std). The heuristic uses privileged pose state. RL variants use a symmetric actor-critic (shared encoder provides auxiliary gradient); BC variants use a PointNet encoder with identical architecture; BC$\to$RL uses an asymmetric actor-critic (frozen BC encoder for actor, state-based MLP critic). RL fails regardless of observation type; for BC, raw PC matches or exceeds segmented PC across all metrics.}
\label{tab:results}
...
RL & Raw PC     & $6.78 \pm 1.01$ & $0.758 \pm 0.072$ & $0.815 \pm 0.039$ & $53.3 \pm 8.7$ & $53.0 \pm 3.9$ & $0.6 \pm 0.6$ \\
\addlinespace
BC & Seg.\ PC   & $9.64 \pm 1.05$ & $0.907 \pm 0.028$ & $0.816 \pm 0.020$ & $70.3 \pm 10.6$ & $96.5 \pm 4.7$ & $0.9 \pm 1.0$ \\
BC & Raw PC     & $9.32 \pm 1.12$ & $0.904 \pm 0.037$ & $0.824 \pm 0.025$ & $84.5 \pm 9.8$ & $\mathbf{98.5 \pm 5.1}$ & $\mathbf{0.7 \pm 0.7}$ \\
\addlinespace
BC$\to$RL & Raw PC & $8.65 \pm 0.97$ & $0.906 \pm 0.027$ & $\mathbf{0.947 \pm 0.016}$ & $86.3 \pm 8.5$ & $98.2 \pm 1.5$ & $1.2 \pm 0.8$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tabular}{@{}llcccccc@{}}
\toprule
Method & Condition & $g$ & $\alpha$ & $\varsigma$ & Grasp succ.\,(\%) & Clear rate\,(\%) & Knocked off\,(\%) \\
\midrule
...
Under the combined reward ($g \cdot \alpha \cdot \varsigma$), the RL policy can achieve high per-grasp scores without clearing the full pile, converging to a targeting pattern that works in dense configurations but does not adapt as the pile thins.
Once the easily accessible logs are exhausted, the fixed strategy produces missed grasps that consume the episode budget.
This is not a perception limitation, as privileged pose observations fare no better (Table~\ref{tab:results}), but a consequence of the exploration landscape: the policy never encounters the diversity of pile states that arise during a complete clearing trajectory.
We also evaluated three reward shaping strategies (Table~\ref{tab:reward_ablation}): removing alignment and stability terms to simplify the reward landscape, normalizing throughput by available logs, and combining tiered clearing bonuses (at 60/75/90\%) with pile-size curriculum. All variants achieved comparable or higher per-grasp reward but none resolved the clearing plateau, suggesting that the bottleneck is exploration rather than reward design.

\begin{table}[t]
\centering
\caption{Reward shaping ablation. All variants use raw PC observations
...
BC initialization provides the trajectory-level coverage that from-scratch RL lacks: the demonstration data spans the full clearing sequence from dense stacks to sparse endgames~\cite{rajeswaran2018learning}.
RL fine-tuning then improves per-grasp execution quality without needing to rediscover the global targeting strategy.
The encoder must remain frozen because the state-based critic provides no gradient signal to the visual pathway (Section~\ref{sec:bcrl}), and low initial noise ($\sigma_0 = 0.05$) prevents RL from overwriting the learned policy.
...
Under our grasp-quality reward (throughput, alignment, stability), from-scratch RL clears only about half the pile regardless of observation type.
