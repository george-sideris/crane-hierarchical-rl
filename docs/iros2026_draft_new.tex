...
\label{sec:obs}
The policy observes a raw point cloud (PCD) of the scene.
...
\textbf{Alignment} ($\alpha$): for each grasped log $i$, we measure the angle between the grapple's jaw axis $\hat{y}_g$ and the log's length axis $\hat{y}_{\ell_i}$ via their dot product ($\hat{y}_g^\top \hat{y}_{\ell_i} = \cos\theta_\alpha$, where $\theta_\alpha$ is the misalignment angle).
...
A shared per-point MLP $h: \mathbb{R}^3 \to \mathbb{R}^{256}$ (layers $3 \!\to\! 64 \!\to\! 128 \!\to\! 256$ with batch normalization and ELU activations) maps each point independently; a symmetric max-pool aggregates the per-point features, and a post-pooling fully connected layer ($256 \!\to\! 256$, BN, ELU) yields the global latent vector:
\begin{equation}
...
(Table~\ref{tab:results}). BC$\to$RL initializes from the trained BC policy and reaches
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{episode_001_pipeline_paper.png}
\caption{Observation pipeline at two episode stages. Each row: (a)~RGB, (b)~depth, (c)~3D back-projection, (d)~FPS point cloud with predicted grasp target (green~=~hit, red~=~miss). Top: dense pile; bottom: sparse endgame.}
...
Alignment remains comparable to BC, so the improvement is driven entirely by more stable grasps.
...
All achieve roughly similar clearing (Table~\ref{tab:results}), confirming that RL's failure is fundamental to the exploration landscape rather than a limitation of the observation representation.
In contrast, observation type does matter for BC: raw PCD outperforms segmented PCD in both clearing and success rate (Table~\ref{tab:results}), showing that segmentation provides no benefit and slightly hurts performance.
...
The best RL variant (Raw PCD) achieves only 53\% clearing with 53.3\% grasp success, compared to 98.5\% and 84.5\% for BC (Table~\ref{tab:results}).
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{episode_001_progression_paper.png}
\caption{Grasp progression for a single BC (PCD) episode clearing 200 logs in 21 grasps. Each panel shows the FPS-downsampled point cloud with the predicted grasp target (green rectangle). Top row: early grasps into the dense pile; bottom row: endgame grasps targeting sparse remaining logs. The pile transitions from a dense ordered stack to a thin irregular layer.}
...
From-scratch RL achieves competitive per-grasp reward but stalls at roughly 53\% clearing regardless of observation type (Table~\ref{tab:results}).
...
RL fine-tuning then optimizes per-grasp alignment and stability without needing to rediscover the global targeting strategy.
...
Raw point clouds achieve comparable clearing via both BC and BC+RL, matching or exceeding segmented PCD (Table~\ref{tab:results}).
