...
\title{BARK: Learning to Clear Dense Log Piles from Vision via Behavior Cloning and Reinforcement Learning}
...
Our framework, BARK, first trains a PointNet-based policy via behavior cloning (BC) on demonstrations from a privileged-state heuristic, then fine-tunes with RL using an asymmetric actor-critic.
...
The resulting policy achieves $98.3$\% pile clearing while improving grasp quality by $16$\% and grasp success rate by $5$ percentage points over the BC initialization, using only depth-derived point clouds.
\end{abstract}
...
We find that from-scratch RL stalls at roughly 60\% pile clearing, a failure that persists across all observation types tested.
...
\toprule
Method & Obs. & Throughput & Align.\,$\alpha$ & Stab.\,$\varsigma$ & Grasp succ.\,(\%) & Clear rate\,(\%) & Knocked off\,(\%) \\
\midrule
Heuristic & Pose & $\mathbf{9.40 \pm 1.17}$ & $\mathbf{0.914 \pm 0.027}$ & $0.789 \pm 0.024$ & $\mathbf{88.4 \pm 10.1}$ & $97.9 \pm 4.4$ & $1.4 \pm 1.6$ \\
\addlinespace
RL        & Raw PCD & \multicolumn{6}{c}{\TODO{Fill after RL (Raw PCD, sym) evaluation}} \\
\addlinespace
BC        & Raw PCD & $9.23 \pm 0.95$ & $0.905 \pm 0.033$ & $0.824 \pm 0.025$ & $82.5 \pm 10.3$ & $\mathbf{98.6 \pm 2.2}$ & $\mathbf{0.8 \pm 0.7}$ \\
\addlinespace
BC$\to$RL & Raw PCD & $8.66 \pm 0.92$ & $0.908 \pm 0.028$ & $\mathbf{0.950 \pm 0.016}$ & $87.0 \pm 9.3$ & $98.3 \pm 1.6$ & $1.2 \pm 1.0$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\label{sec:bcrl_perf}
BC+RL fine-tuning improves grasp quality $q = \alpha \cdot \varsigma$ while preserving BC's full-trajectory clearing ability (Table~\ref{tab:results}).
The most striking gain is in grapple stability, where BC+RL surpasses both BC and the privileged-state heuristic by a wide margin, suggesting that RL optimizes the vertical component of grasp targets to produce more level lifts.
Grasp success rate also improves over BC, reducing wasted grasps, particularly in the endgame where each miss consumes budget.
...
\begin{figure}[t]
\centering
\TODO{Insert BC+RL ablation training curves: mean episode return vs.\ iteration for four variants (frozen $\sigma_0=0.05$, frozen $\sigma_0=0.1$, frozen $\sigma_0=0.3$, unfrozen $\sigma_0=0.05$). Same style as Fig.~\ref{fig:reward_curves}. All runs to 500 iterations.}
...
Conditional per-grasp quality (throughput, alignment, stability) closely matches the heuristic, indicating that the BC policy has captured the expert's targeting strategy: when a grasp succeeds, it is executed as effectively as one chosen with privileged state.
...
Method & Obs. & Throughput & Align.\,$\alpha$ & Stab.\,$\varsigma$ & Grasp succ.\,(\%) & Clear rate\,(\%) & Knocked off\,(\%) \\
\midrule
RL & Pose (128D) & \multicolumn{6}{c}{\TODO{Eval needed}} \\
RL & Seg.\ PCD   & \multicolumn{6}{c}{\TODO{Eval needed (training: $\sim$58.6\%)}} \\
RL & Raw PCD     & \multicolumn{6}{c}{\TODO{Eval needed (training: $\sim$56\%)}} \\
\addlinespace
BC & Seg.\ PCD   & $9.41 \pm 1.19$ & $0.905 \pm 0.033$ & $0.789 \pm 0.024$ & $72.0 \pm 11.8$ & $96.3 \pm 5.0$ & $1.2 \pm 1.4$ \\
BC & Raw PCD     & $9.23 \pm 0.95$ & $0.905 \pm 0.033$ & $0.824 \pm 0.025$ & $82.5 \pm 10.3$ & $98.6 \pm 2.2$ & $0.8 \pm 0.7$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\centering
\caption{Robustness evaluation under domain randomization (DR), observation noise, and their combination. Clearing rate and grasp success rate (mean $\pm$ std over 100 episodes). DR varies log friction, mass, and spawn pattern; observation noise adds Gaussian perturbations to the point cloud.}
\label{tab:robustness}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}llcc@{}}
\toprule
Method & Condition & Clear rate\,(\%) & Grasp succ.\,(\%) \\
\midrule
Heuristic & Nominal        & $97.9 \pm 4.4$ & $88.4 \pm 10.1$ \\
Heuristic & Obs.\ noise    & \TODO{Eval needed} & \TODO{Eval needed} \\
Heuristic & DR             & \TODO{Eval needed} & \TODO{Eval needed} \\
Heuristic & DR + obs.\ noise & \TODO{Eval needed} & \TODO{Eval needed} \\
\addlinespace
BC        & Nominal        & $98.6 \pm 2.2$ & $82.5 \pm 10.3$ \\
BC        & DR             & \TODO{Eval needed} & \TODO{Eval needed} \\
BC        & Obs.\ noise    & \TODO{Eval needed} & \TODO{Eval needed} \\
BC        & DR + obs.\ noise & \TODO{Eval needed} & \TODO{Eval needed} \\
\addlinespace
BC$\to$RL & Nominal        & $98.3 \pm 1.6$ & $87.0 \pm 9.3$ \\
BC$\to$RL & DR             & \TODO{Eval needed} & \TODO{Eval needed} \\
BC$\to$RL & Obs.\ noise    & \TODO{Eval needed} & \TODO{Eval needed} \\
BC$\to$RL & DR + obs.\ noise & \TODO{Eval needed} & \TODO{Eval needed} \\
\bottomrule
\end{tabular}
\end{table*}
...
BC+RL exhibits a reliability--throughput trade-off: the fine-tuned policy favors more conservative grasps that succeed more often and produce more stable lifts, at the cost of slightly fewer logs per grasp.
...
BC+RL fine-tuning improves grasp quality by 16\% over BC while maintaining near-complete clearing, demonstrating that demonstration data provides the exploration curriculum that RL cannot discover on its own.
