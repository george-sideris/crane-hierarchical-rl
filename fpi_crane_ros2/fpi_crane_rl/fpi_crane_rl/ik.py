...
# Arm joint limits for control. These are the machine's limits, which are wider
# than the URDF's: the URDF values are Isaac Sim's envelope and double as a
# conservative safety threshold (slew +/- 1.74533 = 100 deg). The slew's real
# stop is 1.82 rad (104.2 deg), past which the encoder is damaged. Those extra
# 4.2 deg matter at the near end of the rack: at the rack's x the 100 deg cap
# put the floor of reach at y = -0.80 m, the real stop puts it at y = -1.10 m.
ARM_JOINT_LIMITS = [
    (-1.82, 1.82),         # slew
    (-0.383972, 1.309),    # boom
    (-3.08574, 0.035),     # stick
...
class CraneIK:
    ...
    def __init__(self, urdf_path, telescope_active=True):
        ...
        # The optimizer must search the machine's limits, not the URDF's. Without this
        # it rails the joint at the (narrower) URDF stop and returns that pose, which
        # still passes the post-solve limit check below, so an out-of-URDF-range target
        # comes back as a valid solve that misses by however far the rail was short.
