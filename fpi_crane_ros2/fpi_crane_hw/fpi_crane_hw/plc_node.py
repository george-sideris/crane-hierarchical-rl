...
class PLCNode(Node):
    def __init__(self) -> None:
        ...
        # ACK wait per sent packet. 0.2 was too tight: the PLC ACKs late right after finishing a
        # trajectory (comm task busy) and one late ACK rejects the entire next trajectory
        # ("Failed to send trajectory header"). Longer wait is safe: no resend, no duplicates.
        self.declare_parameter("udp_send_timeout", 0.6)
        ...
        # When True, simulate_trajectory keeps the legacy teleop levelling rule
        # that sets grapplecarrier yaw to `-stick - boom`. When False the
        # trajectory's commanded grapplecarrier value is used as-is, which the
        # learned policy needs in order to drive yaw directly.
    ...
    def simulate_teleop(self):
        ...
        # Mast
        self.joint_command.velocity[RobotLinks.MAST] = (
            self.rrc.axes[RRCAxes.MAST] * scale_vel
        )
        self.joint_command.position[RobotLinks.MAST] = (
            current_pos[RobotLinks.MAST]
            + self.joint_command.velocity[RobotLinks.MAST] * dt
        )

        # Boom
        self.joint_command.velocity[RobotLinks.BOOM] = (
            self.rrc.axes[RRCAxes.BOOM] * scale_vel
        )
        self.joint_command.position[RobotLinks.BOOM] = (
            current_pos[RobotLinks.BOOM]
            + self.joint_command.velocity[RobotLinks.BOOM] * dt
        )

        # Stick
        self.joint_command.velocity[RobotLinks.STICK] = (
            self.rrc.axes[RRCAxes.STICK] * scale_vel
        )
        self.joint_command.position[RobotLinks.STICK] = (
            current_pos[RobotLinks.STICK]
            + self.joint_command.velocity[RobotLinks.STICK] * dt
        )

        # Telescope
        self.joint_command.velocity[RobotLinks.TELESCOPE] = (
            self.rrc.axes[RRCAxes.TELESCOPE] * scale_vel
        )
        self.joint_command.position[RobotLinks.TELESCOPE] = (
            current_pos[RobotLinks.TELESCOPE]
            + self.joint_command.velocity[RobotLinks.TELESCOPE] * dt
        )

        # Grapple rotation
        self.joint_command.velocity[RobotLinks.GRAPPLE_BASE] = (
            self.rrc.axes[RRCAxes.GRAPPLE_ROTATION] * scale_vel
        )
        self.joint_command.position[RobotLinks.GRAPPLE_BASE] = (
            current_pos[RobotLinks.GRAPPLE_BASE]
            + self.joint_command.velocity[RobotLinks.GRAPPLE_BASE] * dt
        )
        ...
        self.joint_command.position[RobotLinks.GRIPPER_LEFT] = (
            current_pos[RobotLinks.GRIPPER_LEFT]
            + gripper_vel * dt
        )
        self.joint_command.position[RobotLinks.GRIPPER_RIGHT] = (
            current_pos[RobotLinks.GRIPPER_RIGHT]
            + gripper_vel * dt
        )
            ...
            for joint_index in actuated_joints:
                if joint_index == RobotLinks.TELESCOPE:
                    ...
                    if self.telescope_static:
                        ...
                        position, velocity, acceleration = self.telescope_static_value, 0.0, 0.0
                    else:
                        position = positions[joint_index]
                        velocity = velocities[joint_index]
                        acceleration = accelerations[joint_index]
                else:
                    scale = RAD_TO_DEG * 10.0
                    position = positions[joint_index] + JOINT_SENSOR_OFFSETS[joint_index]
                    velocity = velocities[joint_index]
                    acceleration = accelerations[joint_index]
                data.extend(int(position * scale).to_bytes(2, "big", signed=True))
                data.extend(int(velocity * scale).to_bytes(2, "big", signed=True))
                data.extend(int(acceleration * scale).to_bytes(2, "big", signed=True))
    ...
    def simulate_trajectory(self, points, sequence_id: int) -> None:
        ...
        for i, point in enumerate(points):
            ...
            with self.lock:
                ...
                # Legacy teleop levelling. Off by default so a trajectory can
                # command grapplecarrier yaw directly; set autolevel_grapple
